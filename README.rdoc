...
client project is then:
  ...
  <script src="http://js.example.com/js/js.class/2.1.x/build/min/loader.js" type="text/javascript"></script>
  <script src="http://js.example.com/js/packages.js" type="text/javascript"></script>
...
* Generates a file at <tt>helium-app/public/js/packages.js</tt>, which lists all the files
...
need to configure the package manager; <tt>Helium.use()</tt> must be called to tell it which
projects and which branch of each project we want to use. You can also set the variable
<tt>Helium.PATH</tt> if you've set Helium up to serve script files from a nonstandard
location. By default it is set to:
